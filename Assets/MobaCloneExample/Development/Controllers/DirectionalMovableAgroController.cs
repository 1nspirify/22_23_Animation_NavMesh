...
{
    private const int MinCornersCountInPath = 2;
    ...
    public DirectionalMovableAgroController(IDirectionalMovable movable, Transform target, float agroRange,
        float minDistanceToTarget, NavMeshQueryFilter queryFilter, float timeForIdle)
    {
        _movable = movable;
        _target = target;
    ...
    protected override void UpdateLogic(float deltaTime)
    {
        ...
        if (NavMeshUtils.TryGetPath(_movable.Position, _target.position, _queryFilter, _pathToTarget))
        {
            ...
            if (InAgroRange(distanceToTarget) && EnoughCornersInPath(_pathToTarget) && IdleTimerIsUp())
            {
                _movable.SetMoveDirection(_pathToTarget.corners[TargetCornerIndex] -
                                          _pathToTarget.corners[StartCornerIndex]);
                return;
            }
    ...
    private bool EnoughCornersInPath(NavMeshPath pathToTarget) => pathToTarget.corners.Length >= MinCornersCountInPath;
